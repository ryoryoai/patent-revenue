...
async function requestToken(params) {
  ...
  try {
    ...
    const now = Date.now();
    // 制御文字・BOM・改行を除去（Vercelのfetch/undicがヘッダーバリデーションで弾くため）
    const sanitize = (s) => String(s).replace(/[\x00-\x1f\x7f\ufeff]/g, "").trim();
    tokenState = {
      accessToken: sanitize(data.access_token),
      refreshToken: data.refresh_token ? sanitize(data.refresh_token) : null,
