...
const CHROMIUM_PACK_URL =
  "https://github.com/nicholasgasior/chromium/releases/download/v131.0.1/chromium-v131.0.1-pack.tar";
...
function buildReportHtml(data) {
  ...
  if (report) {
    ...
    if (structured?.inventionSummary?.confidence) {
      const conf = structured.inventionSummary.confidence;
      const confLabel = conf >= 0.7 ? "高" : conf >= 0.4 ? "中" : "低";
      summaryContent += `<p class="confidence">信頼度: ${confLabel}（${Math.round(conf * 100)}%）`;
      if (structured.inventionSummary.evidence?.length > 0) {
        summaryContent += ` / 根拠: ${structured.inventionSummary.evidence.map(e => esc(e)).join(", ")}`;
      }
      summaryContent += `</p>`;
    }
    ...
    // 4. 想定されるライセンス料率
    sections += sectionHtml("4. 想定されるライセンス料率", `<p>${esc(report.royaltyRate)}</p>`);

    // 5. ライセンス可能額の目安
    let valueBracketContent = `<p class="bold">【${esc(report.valueBracket)}】</p>`;
    if (structured?.licenseValueEstimate?.estimatedValue) {
      const est = structured.licenseValueEstimate.estimatedValue;
      const displayValue = est >= 100_000_000
        ? `約${(est / 100_000_000).toFixed(1)}億円`
        : `約${Math.round(est / 10_000).toLocaleString()}万円`;
      valueBracketContent += `<p>推定ライセンス総額: ${displayValue}（収益法による推定）</p>`;
    }
    if (report.valueBracketReason) {
      valueBracketContent += `<p>${esc(report.valueBracketReason)}</p>`;
    }
    sections += sectionHtml("5. ライセンス可能額の目安", valueBracketContent);
    ...
    if (structured?.monetizationMethods?.length > 0) {
      ...
      sections += sectionHtml("6. 収益化手段（スコア順）",
      ...
      if (typeof methods === "object" && methods !== null) {
        sections += sectionHtml("6. 収益化手段", tableHtml([
    ...
    if (structured?.nextActions?.length > 0) {
      ...
      sections += sectionHtml("7. 次の一手（推奨アクション）",
        `<table class="report-table">${actionRows}</table>`
      );
    } else {
      const steps = report.nextSteps;
      if (typeof steps === "object" && steps !== null) {
        sections += sectionHtml("7. 次の一手（推奨アクション）", tableHtml([
...
<div class="content">
  ...
  <div class="footer">
    ...
    <p>&copy; PatentRevenue - 知財を収益に変える</p>
  </div>
</div>
