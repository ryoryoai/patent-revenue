...
const PAGE_MARGIN = { top: "12mm", bottom: "12mm", left: "25mm", right: "25mm" };
...
/** 長文テキストを段落に分割してHTMLにする */
function toParagraphs(text) {
  if (!text) return "";
  // 改行があればそれで分割、なければ2文以上のまとまりで分割
  const raw = String(text);
  let parts;
  if (raw.includes("\n\n")) {
    parts = raw.split(/\n\n+/).filter(Boolean);
  } else {
    // 句点「。」の後で区切り、3文以上を1段落にまとめる
...
function buildReportHtml(data) {
  ...
  const tocItems = [];
  if (report) {
    tocItems.push("1. 発明の概要");
    tocItems.push("2. 発明の強み・優位性");
    tocItems.push("3. ライセンス可能な産業・技術分野");
    tocItems.push("4. 想定されるライセンス料率");
    tocItems.push("6. ライセンス可能額の目安");
    if (report.monetizationMethods || structured?.monetizationMethods) tocItems.push("7. 収益化手段（スコア順）");
    if (report.nextSteps || structured?.nextActions) tocItems.push("9. 次の一手（推奨アクション）");
  }
  const tocHtml = tocItems.length > 0 ? `
    <div class="toc">
      <div class="toc-title">目次</div>
      <ul class="toc-list">
        ${tocItems.map(t => `<li>${esc(t)}</li>`).join("")}
      </ul>
    </div>` : "";
  ...
  if (report) {
    // 1. 発明の概要
    sections += sectionHtml("1. 発明の概要", toParagraphs(report.summary));
    ...
    if (structured?.strengths?.length > 0) {
      ...
      sections += sectionHtml("2. 発明の強み・優位性", toParagraphs(report.strengths));
    }
    ...
    if (structured?.licenseableFields?.length > 0) {
      ...
      sections += sectionHtml("3. ライセンス可能な産業・技術分野", toParagraphs(report.licensableFields));
    }

    // 4. 想定されるライセンス料率
    let royaltyContent = toParagraphs(report.royaltyRate);
    ...
    if (report.valueBracketReason) {
      valueBracketContent += toParagraphs(report.valueBracketReason);
    }
...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  body {
    ...
    font-size: 9.5pt;
    line-height: 1.75;
    padding: 28px 8mm 24px 8mm;
    -webkit-print-color-adjust: exact;
  }

  /* ===== Repeating page footer (all pages via position:fixed) ===== */
  .page-foot {
    position: fixed;
    bottom: 0;
    left: 0;
  ...
  .page-foot span { font-size: 7pt; color: #aaa; }

  /* ===== Repeating page header (all pages via position:fixed) ===== */
  .page-band {
    position: fixed;
    top: 0;
    left: 0;
  ...
  .page-band .pb-brand { font-size: 8pt; font-weight: 700; color: #fff; letter-spacing: 0.05em; }
  .page-band .pb-label { font-size: 7pt; color: rgba(255,255,255,0.7); }

  /* ===== Cover Header (page 1 only, overlays page-band) ===== */
  .cover {
    position: relative;
    z-index: 2;
...
<div class="page-band">
  <span class="pb-brand">PatentRevenue</span>
  <span class="pb-label">特許詳細評価レポート${patent?.id ? " | 特許" + esc(patent.id) : ""}</span>
</div>
<div class="page-foot">
  <span>Confidential</span>
  <span>&copy; PatentRevenue</span>
</div>
