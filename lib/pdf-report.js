...
async function generateReportPdf(data) {
  ...
  try {
    ...
    const pdfBuffer = await page.pdf({
      ...
      headerTemplate: `
        <div style="width:100%;background:#33478e;padding:4px 40px;display:flex;justify-content:space-between;align-items:center;">
          <span style="font-family:'Noto Sans JP',sans-serif;font-size:8px;font-weight:700;color:#fff;letter-spacing:0.05em;">PatentRevenue</span>
          <span style="font-family:'Noto Sans JP',sans-serif;font-size:7px;color:rgba(255,255,255,0.7);">特許詳細評価レポート</span>
        </div>`,
      footerTemplate: `
        <div style="width:100%;text-align:center;padding:4px 0;">
          <span style="font-size:8px;color:#999;">© PatentRevenue - <span class="pageNumber"></span> / <span class="totalPages"></span></span>
        </div>`,
    });
  ...
  if (report) {
    ...
    if (structured?.monetizationMethods?.length > 0) {
      ...
      const litigationWarning = hasLitigation ? `
        <div style="margin-top:12px;padding:10px 14px;background:#fff8f0;border:1px solid #e8a838;border-left:4px solid #d4760a;border-radius:4px;">
          <p style="margin:0;font-size:8pt;font-weight:700;color:#b85c00;">⚠ 訴訟に関する重要な注意</p>
          <p style="margin:4px 0 0;font-size:8pt;color:#7a4a00;line-height:1.6;">本レポートにおける訴訟に関する評価は、法的助言には該当しません。訴訟の成否は個別の事実関係・証拠・裁判管轄等に大きく依存するため、訴訟を検討される場合は必ず知的財産法に精通した弁護士にご相談ください。本レポートの記載内容を根拠とした訴訟提起またはその他の法的手続きにより生じた損害について、当社は一切の責任を負いません。</p>
        </div>` : "";
      sections += sectionHtml("7. 収益化手段（スコア順）",
        `<table class="report-table"><tr><td class="label" style="width:25%">手段</td><td style="width:12%">スコア</td><td>評価</td></tr>${methodRows}</table>${litigationWarning}`
      );
    } else {
...
<div class="content">
  ...
  <div class="footer">
    <p>本レポートはAIによる推定に基づく参考情報であり、法的助言、投資助言、価格保証のいずれにも該当しません。訴訟を含む法的手続きの判断や投資判断の根拠として使用しないでください。意思決定にあたっては、必ず専門家にご相談ください。</p>
