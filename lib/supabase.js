 ...
 */
async function saveLead({ name, companyName, email, source = "patent-value-analyzer", referrer = null, utmData = null, landingPage = null, queryInput = null, useStatus = null, salesRange = null, contribution = null }) {
  ...
  try {
    ...
    if (useStatus) row.use_status = useStatus;
    if (salesRange) row.sales_range = salesRange;
    if (contribution) row.contribution = contribution;
...
async function saveDetailedReportRequest({ leadId, patentId, rank, source, department, phone, supportMethod, desiredPrice, catalogPublishAgreed, privacyAgreed, diagnosisToApplicationSeconds }) {
  const client = getClient();
  if (!client) return null;
  ...
  try {
    ...
    if (phone) { row.phone = phone; row.contact_name = department || null; }
    if (supportMethod) row.support_method = supportMethod;
    if (desiredPrice) row.desired_price = desiredPrice;
    if (catalogPublishAgreed != null) row.catalog_publish_agreed = catalogPublishAgreed;
    if (privacyAgreed != null) row.privacy_agreed = privacyAgreed;
    if (diagnosisToApplicationSeconds != null) row.diagnosis_to_application_seconds = diagnosisToApplicationSeconds;
