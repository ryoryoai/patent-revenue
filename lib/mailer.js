...
async function sendErrorAlertEmail({ to, errorType, message, requestId, url, timestamp }) {
  const html = `
<div style="font-family: 'Noto Sans JP', sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
  <div style="background: #dc2626; color: #fff; padding: 16px 20px; border-radius: 8px 8px 0 0;">
    <h2 style="margin: 0; font-size: 18px;">Patent Value Analyzer - エラー通知</h2>
  </div>
  <div style="border: 1px solid #e5e7eb; border-top: none; padding: 20px; border-radius: 0 0 8px 8px;">
    <table style="width: 100%; border-collapse: collapse; font-size: 14px;">
      <tr><td style="padding: 8px 0; color: #6b7280; width: 120px;">種別</td><td style="padding: 8px 0; font-weight: 600; color: #dc2626;">${errorType}</td></tr>
      <tr><td style="padding: 8px 0; color: #6b7280;">メッセージ</td><td style="padding: 8px 0;">${message}</td></tr>
      <tr><td style="padding: 8px 0; color: #6b7280;">リクエストID</td><td style="padding: 8px 0; font-family: monospace; font-size: 12px;">${requestId || "-"}</td></tr>
      <tr><td style="padding: 8px 0; color: #6b7280;">URL</td><td style="padding: 8px 0;">${url || "-"}</td></tr>
      <tr><td style="padding: 8px 0; color: #6b7280;">発生時刻</td><td style="padding: 8px 0;">${timestamp}</td></tr>
    </table>
  </div>
</div>`;
  const subject = `[Alert] ${errorType} - Patent Value Analyzer`;
  const recipients = Array.isArray(to) ? to : [to];
  for (const addr of recipients) {
    try {
      await sendEmail({ to: addr, subject, html });
    } catch (err) {
      console.warn(`[mailer] alert email failed for ${addr}:`, err.message);
    }
  }
}

module.exports = { sendResultEmail, sendDetailedReportEmail, sendPatentInvalidEmail, sendErrorAlertEmail };
