...
function getConfig() {
  return {
    ...
    token: sanitizeHeaderValue(process.env.V2_API_TOKEN || ""),
...
/**
 * Supabaseのpatentsテーブルから特許リストを取得し、
 * ip-rich-phase2のバッチ分析APIに送信する
 * @param {{ limit?: number, status?: string }} options
 * @returns {Promise<Array | null>}
 */
async function exportPatentsForAnalysis(options = {}) {
  const { getSupabase } = require("./supabase");
  const supabase = getSupabase();
  if (!supabase) {
    console.warn("[v2-client] Supabase not available, skipping export");
    return null;
  }

  const targetStatus = options.status || "登録";

  const { data: patents, error } = await supabase
    .from("patents")
    .select("id, patent_number, normalized_number, title, category, applicant, ipc_codes")
    .eq("status", targetStatus)
    .order("created_at", { ascending: false })
    .limit(options.limit || 100);

  if (error) {
    console.error("[v2-client] failed to fetch patents:", error.message);
    return null;
  }
...
/**
 * phase2の分析結果をpatentsテーブルのdiagnosis_resultに保存
 * @param {string} patentId - patentsテーブルのUUID
 * @param {object} result - phase2の分析結果
 * @returns {Promise<boolean | null>}
 */
async function saveAnalysisResult(patentId, result) {
  const { getSupabase } = require("./supabase");
  const supabase = getSupabase();
  ...
  const { error } = await supabase
    .from("patents")
    .update({ diagnosis_result: result, updated_at: new Date().toISOString() })
    .eq("id", patentId);

  if (error) {
    console.error("[v2-client] failed to save analysis result:", error.message);
  }
  return !error;
}
